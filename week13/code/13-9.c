...
{
	sigorder[number++]=signo;//来一个号码，就存到相关计数器上
}
...
int main(int argc,char*argv[])
{
	int i=0;
	int k=0;
	sigset_t blockmask;//屏蔽所有
	sigset_t emptymask;
	sigset_t pendmask;
	struct sigaction sa;

	sigfillset(&blockmask);
	#ifdef USE_SIGACTION//采用定义的进行调用
	sa.sa_handler=handler;
	sa.sa_mask=blockmask;
	sa.sa_flags=SA_RESTERT;
	#endif
	printf("%s:PID is %d\n",argv[0],getpid());
	for(i=1;i<NSIG;i++)
	{
		if(i==SIGKILL||i==SIGSTOP)//全部频闭，整个操作系统无法控制
		{
			continue;
		}
		#ifdef USE_SIGACTION
	...
	for(i=0;i<number;i++)//在休眠期间，排队的情况打印出来
	{
		if(sigorder[i]!=0)
		{
			printf("#%d:signo = %d\n",i,sigorder[i]);
